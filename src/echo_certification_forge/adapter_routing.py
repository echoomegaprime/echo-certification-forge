...
ACTIVE_MATURITY_STATES = frozenset(
    {"EXPERIMENTAL", "CONFORMANCE_PENDING", "CERTIFIED", "DEGRADED"}
)
...
class RoutingReceipt:
    ...
    def from_envelope(cls, envelope: Mapping[str, Any]) -> "RoutingReceipt":
        unsupported = set(envelope) - {"payload", "signature_b64", "key_id"}
        if unsupported:
...
def _trusted_key(key_id: str, trusted_public_keys: Mapping[str, str]) -> Ed25519PublicKey:
    ...
    if f"ed25519:{sha256_bytes(raw)[:32]}" != key_id:
...
) -> tuple[RoutingReceipt | None, RoutingProofResult | None]:
    if not isinstance(envelope, Mapping):
        return None, RoutingProofResult(
            False,
            "BLOCK",
            (
                {
...
def verify_persona_routing(
    *,
    ...
    """Verify that an enabled persona response used its exact registered LoRA."""
    receipt, failure = _load_receipt(response.get("routing_receipt"), trusted_public_keys)
    ...
    except RoutingProofError as error:
        return RoutingProofResult(
            False,
            "BLOCK",
            ({"name": "routing_receipt_time", "passed": False, "actual": str(error)},),
            payload,
        )
    ...
    request_hash = sha256_json(dict(request_payload))
    _check(checks, "request_sha256", payload.get("request_sha256") == request_hash, payload.get("request_sha256"), request_hash)
    ...
    digest = str(payload.get("selected_adapter_digest", "")).removeprefix("sha256:")
    _check(checks, "selected_adapter_digest", digest == expected.adapter_digest, digest, expected.adapter_digest)
    ...
    _check(checks, "maturity_state", payload.get("maturity_state") == expected.maturity_state, payload.get("maturity_state"), expected.maturity_state)
    _check(checks, "maturity_routable", expected.maturity_state in ACTIVE_MATURITY_STATES, expected.maturity_state, sorted(ACTIVE_MATURITY_STATES))
    ...
    _check(checks, "active_adapter_ids", payload.get("active_adapter_ids") == [expected.adapter_id], payload.get("active_adapter_ids"), [expected.adapter_id])
    ...
    _check(checks, "server_build_digest", len(str(payload.get("server_build_digest", "")).removeprefix("sha256:")) == 64, payload.get("server_build_digest"), "sha256 digest")
    _check(checks, "base_model_digest", len(str(payload.get("base_model_digest", "")).removeprefix("sha256:")) == 64, payload.get("base_model_digest"), "sha256 digest")
...
def verify_unloaded_adapter_failure(
    *,
    ...
    """Verify an inactive enabled adapter fails visibly without base fallback."""
    receipt, failure = _load_receipt(error_response.get("routing_receipt"), trusted_public_keys)
    if failure is not None:
        return failure
    assert receipt is not None
    payload = receipt.payload
