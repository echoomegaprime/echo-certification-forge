    ...
    with tempfile.TemporaryDirectory(
        ...
        def certify(run_id: str, target: TargetIdentity) -> None:
            ...
            e2e_payload = {
                ...
                "canonical_target": (
                    f"https://api.github.com/certforge/{target.identity_digest}"
                ),
        ...
        try:
            ...
            check(
                "A1",
                "uncertified artifact production deployment fails via hook",
                code == 2 and "artifact_not_certified" in out,
                {"exit": code, "stdout": out},
            )

            # bind both certifications over live HTTP (signed with the deployment credential)
            status, body = http(
                "POST", f"{base}/v1/certifications/cert-p6-v1/bindings", sign=True
            )
            bound_v1 = status == 201 and body["artifact_sha256"] == v1.artifact_sha256
            status, body = http(
                "POST", f"{base}/v1/certifications/cert-p6-v2/bindings", sign=True
            )
            bound_v2 = status == 201 and body["artifact_sha256"] == v2.artifact_sha256
            check(
                "A0",
                "certifications bind to exact artifact digests over live HTTP",
                bound_v1 and bound_v2,
                {"v1": bound_v1, "v2": bound_v2},
            )

            # A2 — different digest from the certified artifact must fail
            code, out = run_hook(
                base,
                digest("p6-app-v1-TAMPERED"),
                "staging",
                env_digest,
                rule_digest,
                "deploy-a2",
            ...
            check(
                "A2",
                "different digest from certified artifact fails via hook",
                code == 2 and "artifact_not_certified" in out,
                {"exit": code, "stdout": out},
            )
            ...
            check(
                "A4",
                "production before staging acceptance fails (staging-first)",
                code == 2 and "staging_acceptance_missing" in out,
                {"exit": code, "stdout": out},
            )
            ...
            if staging_ok:
                ...
                prod_admission = (
                    json.loads(out)["admission_id"] if production_ok else None
                )
            check(
                "A3",
                "READY artifact passes staging->production, outcomes via pipeline hook",
                staging_ok and outcome_ok and production_ok,
                {
                    "staging_exit0": staging_ok,
                    "outcome_recorded": outcome_ok,
            ...
            if prod_admission is None:
                raise RuntimeError(
                    "A3 did not produce a production admission; cannot continue"
                )
            ...
            check(
                "A6",
                "failed production deployment produces rollback evidence to last-known-good",
                failure_code == 0
                and candidate.get("artifact_sha256") == v1.artifact_sha256
                and target_info.get("artifact_sha256") == v1.artifact_sha256
                and rolled_code == 0
                and (rolled.get("payload") or {}).get("rollback_to")
                == v1.artifact_sha256,
                {
            ...
            check(
                "A5",
                "revoked certification is denied afterwards (fail-closed lifecycle)",
                code == 2 and "verdict_revoked" in out,
                {"exit": code, "stdout": out},
            )
            ...
            admissions = [
                r for r in audit["records"] if r["record_type"] == "ADMISSION"
            ]
            outcomes = [r for r in audit["records"] if r["record_type"] == "OUTCOME"]
            check(
                "A7",
                "append-only deployment audit chain verifies with full decision history",
                audit["chain_valid"] is True
                and len(admissions) >= 8
                and len(outcomes) >= 5,
                {
            ...
            status1, first = http(
                "POST", f"{base}/v1/hooks/build", payload, signed_headers
            )
            status2, second = http(
                "POST", f"{base}/v1/hooks/build", payload, signed_headers
            )
            ...
            status3, third = http(
                "POST", f"{base}/v1/hooks/build", payload, bad_headers
            )
            check(
                "A8",
                "signed webhook creates run, replay deduplicates, bad signature is 401",
                status1 == 201
                and status2 == 200
            ...
            check(
                "A10",
                "unsigned mutation with tenant header only is rejected 401",
                status_unsigned == 401
                and unsigned_body.get("detail")
                == "deployment_credential_signature_missing",
            ...
            first_status, _ = http(
                "POST", f"{base}/v1/deployments/admissions", replay_body, replay_headers
            )
            replay_status, replay_response = http(
                "POST", f"{base}/v1/deployments/admissions", replay_body, replay_headers
            )
            check(
                "A11",
                "exact replay of a signed mutation is rejected (nonce consumed)",
                first_status == 200
                and replay_status == 401
                and replay_response.get("detail")
                == "deployment_credential_nonce_reused",
                {
                    "first": first_status,
        ...
        code, out = run_hook(
            base, v1.artifact_sha256, "production", env_digest, rule_digest, "deploy-a9"
        )
        check(
            "A9",
            "forge unreachable -> hook fails closed (exit 3), deployment blocked",
            code == 3 and "admission_unavailable" in out,
            {"exit": code, "stdout": out},
        )
