...
VALUES
  ('echo.certforge.r5.submit_async',
   ...
   '{"type":"object","required":["expected_server_build_digest","expected_registry_snapshot_digest","expected_registry_revision","expected_signing_key_id","expected_base_model_digest","expected_base_model_revision","expected_gs343_digest","expected_r2d2_digest","evidence_run_id"],"properties":{"expected_server_build_digest":{"type":"string","pattern":"^[0-9a-f]{64}$"},"expected_registry_snapshot_digest":{"type":"string","pattern":"^[0-9a-f]{64}$"},"expected_registry_revision":{"type":"string"},"expected_signing_key_id":{"type":"string"},"expected_base_model_digest":{"type":"string","pattern":"^[0-9a-f]{64}$"},"expected_base_model_revision":{"type":"string"},"expected_gs343_digest":{"type":"string","pattern":"^[0-9a-f]{64}$"},"expected_r2d2_digest":{"type":"string","pattern":"^[0-9a-f]{64}$"},"evidence_run_id":{"type":"string","pattern":"^[A-Za-z0-9._-]{1,64}$"},"dry_run":{"type":"boolean"}}}'::jsonb,
