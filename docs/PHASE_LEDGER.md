...
| P7 subscriber productization and governance | COMPLETE | `artifacts/p7_acceptance_report.json` (`passed=true`, **37 executable scenarios**, `release_verdict=NOT_READY`); `tests/test_p7_subscriber_governance.py` (**50 passed**, including separate staging and production deploy-environment smokes); full regression **329 passed, 1 skipped**; required P1 acceptance passed (2026-07-25). API intake atomically materializes the deterministic run, BOUND reservation, idempotency record, and exact outbox. Workers reconcile canonical acquired target/environment identities before execution; drift fails closed. Leased heartbeats fence and terminate customer execution on storage loss, and signed completion requires the same active unexpired lease. Staging/prod smoke uses protected distinct peppers and authenticated isolated tenants/projects. | NOT_READY |
...
- P7 subscriber governance: **COMPLETE** — tenant-scoped bearer authorization, role/scope intersection, append-only hash-chained final-outcome audit, authoritative subscription-period metering, ordered lifecycle enforcement, atomic run/reservation/idempotency/outbox materialization, canonical target/environment reconciliation, atomically bound leased worker claims, heartbeat-loss process termination, lease-fenced signing/completion, crash recovery with compensation, execution-boundary version binding, suspension-race closure, transaction-local mutation authorization, risk-state-preserving plan reconciliation, authenticated deploy smoke, resource revocation, and versioned policy/contract are executable and green in `artifacts/p7_acceptance_report.json`
