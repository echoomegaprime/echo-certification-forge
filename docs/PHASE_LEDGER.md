...
| P6 deployment enforcement and platform integration | IMPLEMENTED / PENDING INTEGRATION (branch `feat/p6-deployment-enforcement-fable`) | Live local acceptance 11/11 PASS — `artifacts/p6_acceptance.json` (real uvicorn service + real pipeline-hook subprocess): uncertified/mismatched-digest/pre-staging/revoked/forge-down all DENIED fail-closed; certified staging→production ALLOWED; rollback evidence + tamper-evident audit chain; HMAC webhooks + run dedup; review fixes: HMAC deployment credentials on all mutation endpoints (A10), controller bound to active manifest digest (stale-digest rollover rejected), BEGIN IMMEDIATE linear ledger appends under concurrency, webhook-declared runs reconcile to acquired exact identity before execution/deploy. 29 executable tests (`tests/test_p6_deployment_enforcement.py`, `tests/test_p6_platform_integration.py`); full suite 308 passed (+1 docker-gated skip on HAMMER). Remaining SPEC-P6 breadth: native Git-provider event formats, queue priorities. | NOT_READY |
