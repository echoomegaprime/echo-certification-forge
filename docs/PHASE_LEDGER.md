...
| P6 deployment enforcement and platform integration | IMPLEMENTED / PENDING INTEGRATION (branch `feat/p6-deployment-enforcement-fable`) | Live local acceptance 12/12 PASS — `artifacts/p6_acceptance.json` (real uvicorn service + real pipeline-hook subprocess): uncertified/mismatched-digest/pre-staging/revoked/forge-down all DENIED fail-closed; certified staging→production ALLOWED; rollback evidence + tamper-evident audit chain; HMAC webhooks + run dedup; review fixes: HMAC deployment credentials on all mutation endpoints (A10), controller bound to active manifest digest (stale-digest rollover rejected), BEGIN IMMEDIATE linear ledger appends under concurrency, webhook-declared runs reconcile to acquired exact identity before execution/deploy; adversarial fixes: v2 signed mutations bind tenant+method+canonical path+timestamp+unique nonce+body hash with persistent atomic nonce replay rejection (A11), run/request identity binds server-side active manifest digest (same-manifest-id/new-digest rollover deterministically recertifies), `deploy/enforce_admission.sh` records real authenticated post-deploy outcomes (SUCCEEDED/FAILED/ROLLED_BACK) via hook `outcome` using exact admission_id; release fixes: immutable atomic terminal outcome transitions per admission (FAILED->SUCCEEDED forgery and repeated terminals rejected under BEGIN IMMEDIATE; staging_accepted keyed to the one immutable first successful staging outcome), rollback evidence frozen into the FAILED production record at failure time (ROLLED_BACK accepted only when rollback_to exactly matches the bound candidate — concurrent target drift cannot retarget it), and `deploy/enforce_admission.sh` always executes the real production rollback even when outcome recording fails (3x retried reporting, bound-candidate extraction from the FAILED record, exit 3 fail-closed when reporting is incomplete). 40 executable tests (`tests/test_p6_deployment_enforcement.py`, `tests/test_p6_platform_integration.py`); full suite 319 passed (+1 docker-gated skip on HAMMER). Remaining SPEC-P6 breadth: native Git-provider event formats, queue priorities. | NOT_READY |
...
A completed phase does not upgrade the whole-product verdict. P4 hostile-runner + image supply-chain qualification is now COMPLETE (FORGE gate green 2026-07-20, commit 8c6b30d — validates the 307667a signer image-identity fix and the 25420a8 public-verifier-probe fix; run repeatably via `scripts/run_p4_gate.sh`). P5 now has authoritative GS343/R2D2 identity and negative-control routing proof, but its signed gate remains BLOCK on semantic quality and STABLE-maturity requirements. P6 deployment enforcement is implemented and locally acceptance-proven on branch `feat/p6-deployment-enforcement-fable` (SPEC section 37 acceptance: uncertified fails, mismatched digest fails, valid READY passes; plus staging-first, revocation, rollback evidence, audit chain, signed webhooks + dedup — reproduce via `python scripts/p6_acceptance.py`); it awaits integration into the mainline and FORGE rollout. Remaining fail-closed blockers to product readiness: P5 adapter retraining/promotion, P6 mainline integration + production rollout, P7 subscriber governance, and hosted CI resolution (central `echo.certforge.*` registration is now DONE).
