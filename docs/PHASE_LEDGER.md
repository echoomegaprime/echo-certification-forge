...
| P4 hostile runner, signer image, and supply-chain qualification | COMPLETE — **EVIDENCE NOT RETRIEVABLE** | FORGE gate `passed=True, run_outcome=COMPLETE` — `p4-runs/p4-8c6b30d-rerun7c/p4_hostile_result.json` (2026-07-20, commit 8c6b30d). ⚠️ That artifact exists in neither this repository nor `/home/forge` on the build host, so this COMPLETE mark **cannot be audited against the evidence it cites**. This is not a claim that the qualification never ran — only that its evidence is unavailable for inspection. Produce the artifact or withdraw the mark (queue #26804). | NOT_READY |
